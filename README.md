...
打开页面：用浏览器直接打开 index.html，或用静态服务器（如 VSCode Live Server、python -m http.server）以项目根目录为根路径访问。
Open: Load index.html in a browser, or serve the project root with any static server (e.g. VSCode Live Server, python -m http.server).
